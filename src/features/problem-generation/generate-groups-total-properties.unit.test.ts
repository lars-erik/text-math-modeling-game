...
test('generated groups-total cases satisfy the multiplication relation and validation', () => {
  fc.assert(
    fc.property(generationSeedArbitrary, fc.context(), (seed, context) => {
      ...
      expect(generated.problem.quantities.map((quantity) => quantity.id)).toEqual(
        ['count', 'unitValue', 'total'],
      );
    }),
    { numRuns: 100 },
  );
...
test('generated groups-total cases round-trip through canonical DSL serialization', () => {
  fc.assert(
    fc.property(generationSeedArbitrary, fc.context(), (seed, context) => {
      const generated = generateGroupsTotalCase({
        seed,
...
test('different seeds produce meaningful variation across a sample', () => {
  const generatedCases = Array.from({ length: 20 }, (_, seed) =>
    serializeProblem(
      generateGroupsTotalCase({ seed, config: generationConfig }).problem,
    ),
  );
