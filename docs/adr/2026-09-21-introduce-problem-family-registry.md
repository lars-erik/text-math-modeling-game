...
3. Family x mode capability lives on the Mode axis. `features/puzzle/modes/family-support.ts` declares, per Mode, which families it supports, and the architecture boundary tests mechanically prevent domain modules from importing the puzzle/session/navigation axes.
